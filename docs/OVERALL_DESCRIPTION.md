...
4. `parameters`: the user must define lists of `sensors`, `actuators`, and additional parameters called `other` that are needed by the function.
...
Functions can be executed either as an instantaneous function or as a COMPSs workflow, designed for non-instantaneous, larger tasks. In this case, the computational work is distributed among the available COMPSs agents. To use the COMPSs workflow functionality, the user must specify the `type` field as `workflow`.
