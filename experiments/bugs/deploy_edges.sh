...
DT_SCRIPT_DIR="${SCRIPT_DIR}/../../deployments"
...
DOCKER_IMAGE="${DEPLOYMENT_PREFIX}/edge:debug-matmul"
...
defaults_json="${DT_SCRIPT_DIR}/defaults/setup/edge_default.json"  # Edge default configuration
setup_folder=$(realpath "${DT_SCRIPT_DIR}/${DEPLOYMENT_NAME}/setup")
...
if [ -z "$COMM_SETUP" ]; then
    ...
    deployment_json="${DT_SCRIPT_DIR}/${DEPLOYMENT_NAME}/deployment_setup.json"
else
    # If a communication setup is provided, override the deployment configuration and use the one in the defaults directory
    deployment_json="${DT_SCRIPT_DIR}/defaults/deployment_setup_${COMM_SETUP}.json"
fi

units_deployment="${DT_SCRIPT_DIR}/${DEPLOYMENT_NAME}/default_units.json"
units_default="${DT_SCRIPT_DIR}/defaults/default_units.json"
...
resources_template_path="${DT_SCRIPT_DIR}/templates/resources_template.xml"
project_template_path="${DT_SCRIPT_DIR}/templates/project_template.xml"
...
for label in "${!labels_paths[@]}"; do
    ...
    if [[ "$compss_project" != "null" ]]; then
        project_path="${DT_SCRIPT_DIR}/${DEPLOYMENT_NAME}/project_${label}.xml"
